...
// pruneFileIndex slims down metadata to save token space in tool contexts
func (s *JSONStore) pruneFileIndex(file *types.FileIndex) {
	// Keep Summary, Language, LineCount, SizeBytes, Patterns, RelatedFiles - they are valuable context
	// file.ContentHash = "" // Maybe keep hash for sync later

	// Slim down Exports - we only need Name and Kind for general context
	// We keep Line numbers for now though as they are useful for "go to line"
}
