...
- You just pulled major updates from git
...
func runReindex(cmd *cobra.Command, args []string) {
	...
	// Clean start for index and cache
	indexDir := filepath.Join(tcDir, "index")
	cacheDir := filepath.Join(tcDir, "cache")
	
	fmt.Print("  - Cleaning stale index and cache... ")
