...
import (
	...
	"github.com/teamcontext/teamcontext/internal/git"
...
Example:
  ...
  teamcontext init --force          # WIPE EVERYTHING and start fresh`,
	Run: runInit,
}
...
func init() {
	...
	initCmd.Flags().BoolVar(&forceInit, "force", false, "Force initialization (wipes existing .teamcontext)")
}
...
func runInit(cmd *cobra.Command, args []string) {
	...
	if _, err := os.Stat(tcDir); err == nil {
		if forceInit {
			fmt.Println("Warning: --force flag detected. Wiping existing .teamcontext directory...")
			if err := os.RemoveAll(tcDir); err != nil {
				fmt.Printf("Error wiping directory: %v\n", err)
				return
			}
		} else {
			...
			if hasKnowledge {
				...
				fmt.Println("  - check your .teamcontext/config.json if the path is correct")
				fmt.Println("  - or force a fresh initialization:")
				fmt.Println("  teamcontext init --force")
				fmt.Println("")
				fmt.Println("Use 'teamcontext status' to see current state.")
			} else {
				fmt.Println("  ✓ TeamContext initialized (found .teamcontext directory)")
				fmt.Println("Use 'teamcontext status' to see current state.")
...
`
	...
	fmt.Printf("  2. Start a feature context: teamcontext start <feature-name>\n")
...
func autoConfigureIDEs(projectRoot string) []string {
	...
	binaryPath, _ := exec.LookPath("./teamcontext")
	if binaryPath == "" {
		if bp, err := exec.LookPath("teamcontext"); err == nil {
			binaryPath = bp
		} else {
			fmt.Println("Warning: Could not find teamcontext binary for IDE config")
			binaryPath = "teamcontext" // Fallback to bare command
		}
	}
...
func autoPopulateProjectMetadata(projectRoot, tcDir string) {
	...
	if pkgData, err := os.ReadFile(filepath.Join(projectRoot, "package.json")); err == nil {
		...
		if strings.Contains(pkgStr, "vite") {
