...
func (s *Server) handleToolsList(req *Request) {
	tools := []ToolInfo{
		...
		{
			Name:        "get_code_map",
			Description: "GET PROJECT STRUCTURE grouped by directory with language/export metadata. Useful when you need a compact map with filters.",
			InputSchema: InputSchema{
				Type: "object",
				Properties: map[string]Property{
					"path":      {Type: "string", Description: "Filter to a subdirectory"},
		...
		{
			Name:        "get_skeleton",
			Description: "GET CODE STRUCTURE without implementation bodies. Returns function signatures and type declarations for a file or directory.",
			InputSchema: InputSchema{
				Type: "object",
				Properties: map[string]Property{
					...
					"recursive": {Type: "boolean", Description: "Process subdirectories when path is a directory (default: false)"},
				},
				Required: []string{"path"},
			},
		},
		...
		},
		{
			...
			InputSchema: InputSchema{
				Type: "object",
				Properties: map[string]Property{
					"task": {Type: "string", Description: "Task type: 'add-endpoint', 'add-feature', 'add-service', 'fix-bug', 'refactor', 'add-test', 'add-react-component', 'add-react-hook'"},
