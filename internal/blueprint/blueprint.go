...
import (
	...
	"github.com/teamcontext/teamcontext/internal/imports"
...
const (
	...
	TaskAddReactComponent TaskType = "add-react-component"
	TaskAddReactHook      TaskType = "add-react-hook"
)
...
// maxConventions caps total convention key-value pairs.
const maxConventions = 10
...
func (g *Generator) Generate(taskType TaskType, app, path string) (*Blueprint, error) {
	...
	case TaskAddReactComponent:
		g.generateReactComponentBlueprint(bp)
	case TaskAddReactHook:
		g.generateReactHookBlueprint(bp)
	default:
...
func (g *Generator) getTaskDescription(taskType TaskType) string {
	descriptions := map[TaskType]string{
		...
		TaskAddReactComponent: "Add a new React functional component with props and styling",
		TaskAddReactHook:      "Add a new React custom hook for reusable logic",
	}
...
func (g *Generator) generateReactComponentBlueprint(bp *Blueprint) {
	examples := g.findReactComponentExamples(bp.App)
	bp.Examples = examples
	if len(examples) > 0 {
		bp.Confidence += 0.3
		bestExample := examples[0]
		exPath := filepath.Join(g.projectRoot, bestExample.Path)
		featureName := g.extractFeatureNameFromFile(bestExample.Path)
		bp.Snippets = map[string]*SnippetEntry{
			"component": g.extractSingleSnippet(exPath, featureName, "Functional component pattern"),
		}
		bp.Imports = g.extractImportsFromFile(exPath, featureName)
	}

	bp.FilePattern = &FilePattern{
		BasePath: g.inferBasePath(bp.App, "component"),
		Files:    []string{"{Name}.tsx", "{Name}.css", "index.ts"},
	}

	bp.Conventions = g.detectConventions(bp.App)
	bp.Checklist = []string{
		"Define interface for component Props",
		"Implement functional component using PascalCase",
		"Use standard hooks (useState, useEffect) if needed",
		"Apply styling using CSS modules or established pattern",
		"Export component and add to index.ts if in a library",
	}
}

func (g *Generator) generateReactHookBlueprint(bp *Blueprint) {
	examples := g.findReactHookExamples(bp.App)
	bp.Examples = examples
	if len(examples) > 0 {
		bp.Confidence += 0.3
		bestExample := examples[0]
		...
		featureName := g.extractFeatureNameFromFile(bestExample.Path)
		bp.Snippets = map[string]*SnippetEntry{
			"hook": g.extractSingleSnippet(exPath, featureName, "Custom hook pattern"),
		}
		bp.Imports = g.extractImportsFromFile(exPath, featureName)
	}

	bp.FilePattern = &FilePattern{
		BasePath: g.inferBasePath(bp.App, "hook"),
		Files:    []string{"use{Name}.ts"},
	}

	bp.Conventions = g.detectConventions(bp.App)
	bp.Checklist = []string{
		"Start hook name with 'use'",
		"Extract reusable state or side-effect logic",
		"Return state, dispatchers, or event handlers",
		"Add unit tests using @testing-library/react-hooks or similar",
	}
}
...
func (g *Generator) reactComponentPattern(app string) *FilePattern {
	return &FilePattern{
		BasePath: g.inferReactBasePath(app, "component"),
		Files: []string{
			"{Name}.tsx",
			"{Name}.module.css",
			"index.ts",
...
func (g *Generator) reactHookPattern(app string) *FilePattern {
	return &FilePattern{
		BasePath: g.inferReactBasePath(app, "hook"),
		Files: []string{
			"use{Name}.ts",
			"use{Name}.test.ts",
		},
...
func (g *Generator) reactPagePattern(app string) *FilePattern {
	return &FilePattern{
		BasePath: g.inferReactBasePath(app, "page"),
		Files: []string{
			"{Name}.tsx",
			"{Name}.module.css",
		},
...
func (g *Generator) inferReactBasePath(app, kind string) string {
	...
	// Default React layout
	switch kind {
	case "component":
		return "src/components/{name}/"
	case "hook":
		return "src/hooks/{name}/"
	case "page":
...
func (g *Generator) findReactComponentExamples(app string) []Example {
	searchPaths := []string{
		filepath.Join(g.projectRoot, "src", "components"),
		filepath.Join(g.projectRoot, "components"),
		filepath.Join(g.projectRoot, "src", "pages"),
		filepath.Join(g.projectRoot, "pages"),
	}
	patterns := []*regexp.Regexp{
		regexp.MustCompile(`^[A-Z]\w+\.(tsx|jsx)$`),
	}
	return g.findExamplesByPathsAndPatterns(searchPaths, patterns, " React component")
}

func (g *Generator) findReactHookExamples(app string) []Example {
	searchPaths := []string{
		filepath.Join(g.projectRoot, "src", "hooks"),
		filepath.Join(g.projectRoot, "hooks"),
		filepath.Join(g.projectRoot, "src", "logic", "hooks"),
	}
	patterns := []*regexp.Regexp{
		regexp.MustCompile(`^use[A-Z]\w+\.(ts|js)$`),
	}
	return g.findExamplesByPathsAndPatterns(searchPaths, patterns, " custom hook")
}

func (g *Generator) findExamplesByPathsAndPatterns(paths []string, patterns []*regexp.Regexp, descSuffix string) []Example {
	var examples []Example
	type exampleWithMtime struct {
		example Example
		mtime   int64
	...
	for _, searchPath := range paths {
		...
		filepath.Walk(searchPath, func(path string, info os.FileInfo, err error) error {
			...
			for _, p := range patterns {
				if p.MatchString(filepath.Base(path)) {
					matched = true
					break
				}
			}
			...
			if matched {
				...
				candidates = append(candidates, exampleWithMtime{
					example: Example{
						Path:        relPath,
						Description: filepath.Base(path) + descSuffix,
					},
					mtime: info.ModTime().Unix(),
				})
			}
	...
	for i := 0; i < len(candidates) && i < maxExamples; i++ {
		examples = append(examples, candidates[i].example)
	}
