...
var (
	...
	reactRoutePattern = regexp.MustCompile(`<Route\s+[^>]*path=['"]([^'"]+)['"]\s+[^>]*element=\{<([^/\s>]+)`)
)
...
func ExtractAPISurface(dirPath string, appName string) (*APISurface, error) {
	...
	err := filepath.Walk(dirPath, func(path string, info os.FileInfo, err error) error {
		...
		case ".ts", ".js", ".tsx", ".jsx":
			...
			extractReactRoutes(text, path, surface)
		case ".go":
...
func extractReactRoutes(content string, filePath string, surface *APISurface) {
	matches := reactRoutePattern.FindAllStringSubmatchIndex(content, -1)
	for _, match := range matches {
		if len(match) >= 6 {
			path := content[match[2]:match[3]]
			element := content[match[4]:match[5]]
			line := strings.Count(content[:match[0]], "\n") + 1
