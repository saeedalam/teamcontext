...
var (
	...
	// React patterns
	reactComponent = regexp.MustCompile(`(?m)^(\s*)(export\s+)?(const|function)\s+([A-Z]\w+)\s*(?:=\s*(?:async\s+)?\([^)]*\)|<[^>]+>\s*\([^)]*\))\s*(?::\s*[^=]+)?\s*=>\s*(?:<|\()|(?m)^(\s*)(export\s+)?(async\s+)?function\s+([A-Z]\w+)\s*\(`)
	reactHook      = regexp.MustCompile(`(?m)^(\s*)(export\s+)?(const|function)\s+(use[A-Z]\w+)`)
)
...
func parseTypeScript(content string, skeleton *types.CodeSkeleton) {
	...
	for lineNum, line := range lines {
		...
		if m := tsFunction.FindStringSubmatch(line); m != nil {
			...
			// React tagging
			if reactComponent.MatchString(line) {
				fn.Tags = append(fn.Tags, "ReactComponent")
			} else if reactHook.MatchString(line) {
				fn.Tags = append(fn.Tags, "Hook")
			}
		...
		if m := tsArrowExport.FindStringSubmatch(line); m != nil {
			...
			// React tagging for arrow exports
			if reactComponent.MatchString(line) {
				fn.Tags = append(fn.Tags, "ReactComponent")
			} else if reactHook.MatchString(line) {
				fn.Tags = append(fn.Tags, "Hook")
...
func FormatSkeleton(sk *types.CodeSkeleton) string {
	...
	for _, cls := range sk.Classes {
		...
		if len(cls.Tags) > 0 {
			sb.WriteString("// Tags: " + strings.Join(cls.Tags, ", ") + "\n")
		}
	...
	for _, fn := range sk.Functions {
		...
		if len(fn.Tags) > 0 {
			sb.WriteString("// Tags: " + strings.Join(fn.Tags, ", ") + "\n")
		}
