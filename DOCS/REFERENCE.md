...
4. [All MCP Tools (57)](#4-all-mcp-tools-57)
...
This does everything. If a project is already initialized, use `--force` to wipe and start fresh:

```bash
teamcontext init --force
```
...
|---------|-------------|-------------|
| `teamcontext init` | Initialize project (index + IDE setup + git analysis). Use **--force** to completely wipe and start fresh. | First time in a project or when a full reset is needed. |
| `teamcontext reindex` | Clean re-index (wipes cache and file index, preserves knowledge). | After codebase changes or to fix a corrupted index. |
...
### Token-Saving Tools (8 tools)

**`get_signature`** — Live file signature (~90% token savings)
```
"Show me signatures for auth.service.ts"
→ file: "auth.service.ts" (filename or partial path)
→ Returns classes, functions, and imports for the matched file
```
...
### High-Impact Extraction (4 tools)

**`get_blueprint`** — Task blueprint for implementation
```
"Create a blueprint for adding an endpoint in notification service"
→ task: "add-endpoint", app: "notification"
→ Returns conventions, related files, starter snippets, risks, and checklist
```
Supported tasks: `add-endpoint`, `add-feature`, `add-service`, `fix-bug`, `refactor`, `add-test`, `add-react-component`, `add-react-hook`

React examples:
```
"Create a blueprint for a new profile card component"
→ task: "add-react-component", app: "web-app"

"Create a blueprint for a reusable auth hook"
→ task: "add-react-hook", app: "web-app"
```
...
→ Returns: compact tree text from precomputed index (root view is depth-limited)
→ Use path to zoom into subdirectories for more detail
```

**`get_code_map`** — Directory-grouped indexed file map
```
"Show me indexed Go files under internal/mcp"
→ path: "internal/mcp", language: "go", max_depth: 2, limit: 100
→ Returns files grouped by directory with language and export metadata
```
...
→ path: "src/controllers/payment.controller.ts", direction: "forward", depth: 3
